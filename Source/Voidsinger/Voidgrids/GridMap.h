...
public:
	...
	{
		GridInfo = TArray<TPair<FIntPoint, ValueType>>();
	...
	TGridMap(TArray<TPair<FIntPoint, ValueType>> Values, FIntPoint GridDimensions = FIntPoint(250))
	{
	...
	TArray<ValueType> GenerateValueArray()
	{
		...
		for (TPair<FIntPoint, ValueType> GridInfoPair : GridInfo)
		{
			Values.Emplace(GridInfoPair[1]);
		}

		return Values;
	}

	TArray<FIntPoint> GenerateKeyArray()
	{
		TArray<ValueType> Keys = TArray<ValueType>();

		for (TPair<FIntPoint, ValueType> GridInfoPair : GridInfo)
		{
			Keys.Emplace(GridInfoPair[0]);
		}
	...
	bool PointsConnected(FIntPoint StartPoint, FIntPoint EndPoint)
	{
		return PointsConnected(StartPoint, EndPoint, AlwaysConnect);
	}
	bool PointsConnected(FIntPoint StartPoint, FIntPoint EndPoint, TArray<FIntPoint>& ConnectivityArray)
	{
...
private:
	const static bool ComparePair(TPair<FIntPoint, ValueType>& A, TPair<FIntPoint, ValueType>& B)
	{
	...
	const static int32 LocationToRelativeValue(FIntPoint Location)
	{
	...
	const static FIntPoint RelativeValueToLocation(int32 Index)
	{
