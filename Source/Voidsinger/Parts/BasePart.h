...
/**
 *  Comment what the class is -Mabel
 */
...
public:
	...
	//Constructor
	//If you're gonna get on me about "constructor" being a bad comment then i'm gonna get on you about that -Mabel
	UBasePart();
	...
	//for internal use, then this should be a private function. -Mabel
	UFUNCTION()
	...
	//This is called from part grid then, after it's placed? asking for clarification -Mabel
	UFUNCTION()
	void InitializeFunctionality();
	
	//Used to get world. Also gives child BPs acces to the Kismet Systems Library
	//"Comment not helpful. What does the GetWorld() functionality mean? - Liam Suggestion" -Mabel
	UWorld* GetWorld() const;

	//Destroys the part. Does not call OnDestroyed()
	//Why would this not call "on destroyed"? what other way is there for destroying parts? I'm confused. maybe this should be
	//like "DestroyBecauseMerged" or something (horrible name, think of something better than that, but y'know) -Mabel
	UFUNCTION()
	void DestroyPart();
...
public:
	...
	//these functions need the world "part" in their name. It should already be obvious that you are getting the part's
	//location because these functions are defined on the part. -Mabel
	UFUNCTION(BlueprintPure)
	...
	//Either the location getters or these rotation getters should be renamed for consistency. -Mabel
	UFUNCTION(BlueprintPure)
	...
	//to "GetPixelMass" or something along those lines. -Mabel
	UFUNCTION(BlueprintPure)
	float GetMass();

	//Gets the part grid that this part is attached to
	//This can be achieved with "GetOuter" and isn't a necessary function -Mabel
	UFUNCTION(BlueprintPure)
	...
	//"GetOuter()", so this isn't a necessary function either -Mabel
	UFUNCTION(BlueprintPure)
	...
	//Comment technicaly false. It finds the *first* resource system ...
	...
	// end up doing it
	//so this function might not make sense -Mabel
	UFUNCTION(BlueprintPure)
...
protected:
	...
	//Either this comment or the varibale should be renamed. either "Stores the desired shape of the part" or "DefaultShape" -Mabel
	...
	//Why is this comment calling "PartName" strength of the part -Mabel
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly)
	FText PartName;
	...
	//Strength of the part
	//Why is this comment calling "PartIcon" strength of the part - Mabel
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly)
	UTexture2D* PartIcon;
...
private:
	...
	// Why is this commented out? delete if it's not needed anymore
	//UPROPERTY()
